...
public record OptionWithDefaultDTO(long id, String name, boolean defaultValue) {

    public static OptionWithDefaultDTO fromEntity(ChosenOption option) {
        return new OptionWithDefaultDTO(option.originalId(), option.name(), option.isSelected());
    }

    public static OptionWithDefaultDTO fromEntity(ItemOption option) {
        return new OptionWithDefaultDTO(option.getOption().getId(), option.getOption().getName(), option.isDefaultEnabled());
    }
